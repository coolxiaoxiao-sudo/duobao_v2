...
def check_six_dangers(code: str, name: str) -> dict:
    """六大高危雷区检测"""
    dangers = []
    try:
...
def analyze_volume_structure(code: str) -> dict:
    """四大量价结构识别"""
    try:
...
def linkage_validation(code: str, industry: str) -> dict:
    """大盘-板块-个股联动验证"""
    try:
...
def full_danger_check(code: str, name: str, industry: str) -> dict:
    """完整雷区检查"""
    return {
...
def batch_danger_check() -> dict:
    results = {s["code"]:full_danger_check(s["code"],s["name"],s.get("industry","")) for s in config.stocks}
    with open(os.path.join(DANGER_DIR,f"danger_{datetime.now().strftime('%Y%m%d')}.json"),"w",encoding="utf-8") as f:
